...
//        $this->options['expire'] = isset($expire) ? (int) $expire : (int) ini_get('session.gc_maxlifetime');
        $this->options['expire'] = isset($options['expire']) ? (int) $options['expire'] : C('SESSION_EXPIRE');
        $this->options['prefix'] = isset($options['prefix']) ? $options['prefix'] : C('SESSION_PREFIX');
        $this->handler = new \Redis;
        $this->index = C('SESSION_DB_INDEX') ? C('SESSION_DB_INDEX') : 0;
    }
    ...
    public function read($sessID) {
        $this->connect(0);
        $this->handler->select($this->index);
